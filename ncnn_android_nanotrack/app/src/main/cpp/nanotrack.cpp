...
//NanoTrack::NanoTrack(std::string model_backbone, std::string model_head)
//{
//    this->load_model(model_backbone, model_head);
//    create_window();
//    create_grids();
//}
...
int NanoTrack::load_model(AAssetManager* mgr, const char* modeltype, bool use_gpu){

    this->net_backbone.load_param(mgr,"nanotrack_backbone_sim-opt.param");
    this->net_backbone.load_model(mgr,"nanotrack_backbone_sim-opt.bin");

    this->net_head.load_param(mgr,"nanotrack_head_sim-opt.param");
    this->net_head.load_model(mgr,"nanotrack_head_sim-opt.bin");
...
int NanoTrack::draw(cv::Mat& rgb)
{
//    if(!this->is_init)
//    {
//        rgb.copyTo(temp);//初始化图像
//    }

    rgb.copyTo(temp); //保存图像随时用于初始化模版

    cv::rectangle(rgb, cv::Rect(this->state.target_pos.x - this->state.target_sz.x / 2, this->state.target_pos.y - this->state.target_sz.y / 2, this->state.target_sz.x, this->state.target_sz.y),
...
cv::Mat NanoTrack::get_subwindow_tracking(cv::Mat im, cv::Point2f pos, int model_sz, int original_sz,cv::Scalar channel_ave)
{
    ...
    if (top_pad > 0 || left_pad > 0 || right_pad > 0 || bottom_pad > 0)
    {
        ...
        cv::copyMakeBorder(im, te_im, top_pad, bottom_pad, left_pad, right_pad, cv::BORDER_CONSTANT, channel_ave);// 这里不是应该用平均值填充么??
